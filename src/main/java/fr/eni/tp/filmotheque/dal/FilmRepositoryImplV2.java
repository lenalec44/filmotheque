...
//@Repository
public class FilmRepositoryImplV2 { /*implements FilmRepository {

	private static String SELECT_FILM =
			"select f.id as id, titre, annee, duree, "
			+ " synopsis, genre_id, realisateurid,"
			+ " libelle, nom, prenom "
	...
	public FilmRepositoryImplV2(NamedParameterJdbcTemplate namedJdbcTemplate) {
...
//	}
	...
	//Version appel procédure stockée
	@Override
	public List<Film> findAllFilms() {
		
		String sql = "SELECT * FROM get_film_details()";
	...
	public void saveFilm(Film film) {
		...
		if(film.getActeurs().size()>0) {
			...
			namedJdbcTemplate.batchUpdate(sql, acteursParameters);
